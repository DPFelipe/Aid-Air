...
<body>
<head>
<title>AIR-AIR Account Management</title>
<meta charset="UTF-8">
...
<nav class="w3-sidebar w3-red w3-collapse w3-top w3-large w3-padding" style="z-index:3;width:20%;font-weight:bold;" id="mySidebar"><br>
  ...
  <div id="demoAcc" class="w3-hide w3-white w3-card">
    <a href="./sampleedit.php" class="w3-bar-item w3-button w3-hover-white">Admin Accounts</a>
...
<h1 class="w3-xxxlarge w3-text-red"><b>Edit Admin Info - <?php echo $_GET["Admin_ID"]; ?></b></h1>
...
if(!empty($_POST["modify"])) {
  $pdoQuery=$pdoConnect->prepare("UPDATE admin_info SET Password = '" . $_POST['Password'] . "' , LastName = '" .  $_POST['LName']  . "' , FirstName = '" .  $_POST['FName']  . "' WHERE Admin_ID = " . $_GET["Admin_ID"]);
  $pdoResult = $pdoQuery->execute();
  if($pdoResult) {
    echo "Success! <a href = '../admin/newadmin.php'>Back to main page..</a>";
  }
}
    $pdoQuery = $pdoConnect->prepare("SELECT * FROM admin_info where Admin_ID=" . $_GET["Admin_ID"]);
    $pdoQuery->execute();
